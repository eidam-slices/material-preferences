package cz.eidam.demo_shared

object PreferenceKeys {
    const val EXAMPLE_SWITCH = "example_switch"
    const val EXAMPLE_SLIDER = "example_slider"
    const val EXAMPLE_RANGE_SLIDER = "example_range_slider"
    const val EXAMPLE_CHOICE = "example_choice"
    const val EXAMPLE_MULTI_CHOICE = "example_multi_choice"
    const val EXAMPLE_TEXT_FIELD = "example_text_field"
    const val EXAMPLE_INLINE_TEXT_FIELD = "example_inline_text_field"
}